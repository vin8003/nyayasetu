...
| Schema | `migrations/*.sql`, applied at PGLite boot or `npm run db:migrate` |
...
src/
  ...
  lib/
    ...
    practice/             types, store, workflow, sample, extract-order, hearing-brief
    research/             run, verify, letter, files, courts, copy, store
```
...
- A second search client besides `src/lib/research/run.ts`
